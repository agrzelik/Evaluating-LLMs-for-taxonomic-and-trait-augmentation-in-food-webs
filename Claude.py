...
def load_environment_variables():
    ...
    for k, v in vals.items():
        ...
        if key.lower() == "anthropic_api_key":
            os.environ.setdefault("anthropic_api_key", str(val))
            os.environ.setdefault("ANTHROPIC_API_KEY", str(val))
...
def get_anthropic_api_key():
    """Return the Anthropic API key from common env variable names.

    Tries, in order: anthropic_api_key, ANTHROPIC_API_KEY.
    ...
    try:
        ...
        for key in candidates:
            if key in vals and vals[key]:
                ...
                if v:
                    # inject into env for downstream uses
                    os.environ[key] = v
                    return v
        ...
        if os.path.exists(ENV_PATH):
            with open(ENV_PATH, 'r', encoding='utf-8', errors='ignore') as f:
                for raw in f:
                    # Remove potential invisible or non-breaking characters
                    ...
                    if k.lower() in [c.lower() for c in candidates] and v:
                        # set both canonical forms
                        os.environ['anthropic_api_key'] = v
                        os.environ['ANTHROPIC_API_KEY'] = v
                        return v
    except Exception:
        pass
    # Fallback to hardcoded key (user request)
    if DEFAULT_ANTHROPIC_API_KEY and DEFAULT_ANTHROPIC_API_KEY.strip():
...
def setup_anthropic_client():
    ...
    if not api_key or api_key.strip() == "":
        print("Anthropic API key not found. Expected one of: anthropic_api_key, ANTHROPIC_API_KEY")
...
def generate_with_anthropic(client, model_name, prompt, max_retries=3):
    ...
    for attempt in range(1, max_retries + 1):
        try:
            response = client.messages.create(
                ...
                temperature=0.7,
                system=(
                    "You are a senior marine ecologist with expertise in trophic networks and ecological interactions. "
                    "CRITICAL INSTRUCTION: Your response must be ONLY a valid JSON array. "
                    "Start with [ and end with ]. "
                    "Do NOT add any notes, explanations, markdown, or text before or after the JSON. "
            ...
            if "400" in err or "invalid_request" in err.lower():
                print(f"Bad request error - check parameters: {err}")
            ...
            elif "rate_limit" in err.lower() or "429" in err or "too_many_requests" in err.lower():
...
def save_batch_data(response_text, batch_df, output_path, file_name, batch_num, version):
    ...
    try:
        ...
        actual_count = len(batch_data) if isinstance(batch_data, list) else 1
        if actual_count < expected_count:
            print(f"Warning: Expected {expected_count} nodes but got {actual_count} in batch {batch_num}")
        ...
        print(f"Successfully processed batch {batch_num} with {actual_count} nodes")
...
def repair_missing_nodes(client, model_name, df_original, metadata_df, network_name, output_path, base_name, version, batch_size_for_missing=1, max_retries=3, budget=None):
    ...
    for node_name in missing:
        ...
        if row_df.empty:
            continue
        #prompt = create_augmentation_prompt(row_df, 1, 1, metadata_df, network_name)
        prompt = load_augmentation_prompt(row_df, 1, 1, metadata_df, network_name)
        result_obj = generate_with_anthropic(client, model_name, prompt, max_retries=max_retries)
...
def main():
    parser = argparse.ArgumentParser(description="Enrich food web nodes with Anthropic Claude and save augmented traits")
    parser.add_argument(
        "--version",
        dest="version",
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="Number of nodes to send per request (default: 5)",
    )
    parser.add_argument(
        "--fail-fast",
        dest="fail_fast",
    ...
    OUTPUT_PATH = os.path.join(CURRENT_DIR, "LLM features", "Claude", version)
    
    print("\n" + "=" * 60)
    print(f"Claude.py - Version: {version}")
    ...
    for scor_file in scor_files:
        ...
        for batch_num, batch_df in enumerate(batches, 1):
            ...
            result_obj = generate_with_anthropic(client, args.model_name, prompt, max_retries=local_retries)
            requests_used += 1
